 @if(auth()->check())
                        <span class="post-save text-muted tooltip-test float-right" data-toggle="tooltip" data-original-title="Save this post!">
                            @if ($post->saves->contains('user_id', auth()->user()->id))
                                <button class="unsave-btn change save" data-post-id="{{ $post->id }}" data-save-url="{{ route('posts.save', $post) }}" data-unsave-url="{{ route('posts.unsave', $post) }}"><i class="fa fa-bookmark text-warning"></i></button>
                                <button class="save-btn change save" data-post-id="{{ $post->id }}" data-save-url="{{ route('posts.save', $post) }}" data-unsave-url="{{ route('posts.unsave', $post) }}" style="display: none;"><i class="fa fa-bookmark"></i></button>
                            @else
